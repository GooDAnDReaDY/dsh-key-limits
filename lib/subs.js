...
//   subs.json  { credentials: { "<subId>": "<secret>" , ...}, meta: {...} }
//
...
export class CredentialStore {
  ...
  _save() {
    ...
    for (const [id, v] of this.creds) {
      creds[id] = { provider: v.provider, kind: v.kind, secret: v.secret, extra: v.extra || '', label: v.label || '', credentialRef: v.credentialRef || '', updatedAt: v.updatedAt }
    }
    try { writeFileSync(this.path, JSON.stringify({ credentials: creds, meta: this.meta }, null, 2)) } catch { /* best-effort */ }
  }
  list() {
...
async function fetchOpenCodeGoQuota(secret, workspaceId) {
  ...
  const res = await fetch(url, { method: 'GET', headers: { cookie: `auth=${auth}`, accept: 'text/html' } })
  ...
  if (!usage.rolling5h) {
    ...
    return makeError(looksLikeSubscribe
      ? 'OpenCode GO: нет активного плана (страница подписки, usage недоступен)'
      : bareId === 'go'
        ? 'OpenCode GO: неверный workspace (получен "go" — укажите wrk_... или полный URL /workspace/wrk_.../go)'
        : 'OpenCode GO: не удалось распарсить usage из HTML')
  }
  return {
...
async function fetchOllamaQuota(apiKey, sessionCookie) {
  ...
  return cachedFetch(`ollama:${md8(key)}:${md8(cookie)}`, 60_000, async () => {
    ...
    if (!key) metadataError = 'api key not configured'
      ...
      try {
        const metaRes = await fetch('https://ollama.com/api/me', {
          method: 'POST',
          headers: {
    ...
    if (!cookie) usageError = 'session cookie not configured'
      ...
      try {
        const usageRes = await fetch('https://ollama.com/settings', {
          method: 'GET',
          headers: {
            cookie,
            accept: 'text/html',
...
export async function refreshSubscriptionEntry(store, entry) {
  ...
  try {
    ...
    if (!cred || !cred.secret) return { id: entry.id, name: entry.provider, kind: def.kind, status: "error", message: "нет учётной записи", checkedAt: Date.now() }
