...
export class CredentialStore {
  ...
  _save() {
    ...
    for (const [id, v] of this.creds) {
      const ref = v.credentialRef || ('DSH_KEY_LIMITS_' + String(id || 'sub')).toUpperCase().replace(/[^A-Z0-9_]+/g, '_')
      creds[id] = { provider: v.provider, kind: v.kind, secret: '', extra: v.extra || '', label: v.label || '', credentialRef: ref, updatedAt: v.updatedAt }
    }
    try {
