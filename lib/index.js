...
export function apply(ctx, config) {
  ...
  try {
    const sctx = ctx.extend({ name: SETTINGS_NS })
    sctx.settings.register(SETTINGS_NS, Config, { base: config })
  } catch (e) {
  ...
  function upsertSubFromBody(body) {
    ...
    if (!secret && !prev) return { error: 'secret required for new subscription' }
    credStore.upsert(id, {
      provider,
      kind: def.kind,
      secret: secret || (prev && prev.secret) || '',
      extra,
      label,
      credentialRef: String(body.credentialRef || (prev && prev.credentialRef) || '').trim(),
    })
    return { ok: true, id }
  }
  ...
  ctx.effect(() => ctx.webServer.register({
    ...
    handler: async (req, res) => {
      ...
      return json(res, 200, { ui: cfg.ui || {}, refreshHours: cfg.refreshHours })
    },
  }), 'key-limits: config')
    ...
    handler: async (req, res) => {
      try {
        ...
        if (req.method === 'POST') {
          ...
          const r = upsertSubFromBody(body)
