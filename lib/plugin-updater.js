// lib/plugin-updater.js — loopback updater for @goodandready-private/dsh-key-limits
...
async function status(options, target) {
  ...
  const latest = await latestVersion(options.packageName, options.registry ?? 'https://npm.pkg.github.com')
  return {
...
async function installExact(target, packageSpec, options) {
  ...
  await new Promise((resolvePromise, reject) => {
    const child = spawn(process.execPath, [
      ...
      `--registry=${options.registry ?? 'https://npm.pkg.github.com'}`,
    ], {
