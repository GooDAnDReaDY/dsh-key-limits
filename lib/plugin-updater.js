// lib/plugin-updater.js — loopback updater for @goodandready/dsh-key-limits
...
/**
 * @internal Exported for isolated security predicate testing in test/updater.test.mjs.
 * Enforces loopback origins and update headers to prevent CSRF attacks.
 */
...
/**
 * @internal Exported for semver and prerelease comparison verification in test/updater.test.mjs.
 */
...
async function latestVersion(packageName, registry) {
  ...
  try {
    const regUrl = registry.replace(/\/$/, '')
    const response = await fetch(`${regUrl}/${encodeURIComponent(packageName)}/latest`, {
...
export function registerPluginUpdater(ctx, options) {
  ...
  if (typeof ctx?.webServer?.register !== 'function') {
    ctx?.logger?.warn?.(`${options.packageName}: webServer service unavailable, updater skipped`)
    return () => {}
  }
  return ctx.webServer.register({
    ...
    handler: async (request, response) => {
      try {
        ...
        ctx?.logger?.warn?.(`${options.packageName}: plugin updater failed: ${String(error)}`)
