...
export function loadSubs(dir) {
  ...
  try {
    ...
    const subscriptions = []
    for (const [id, v] of Object.entries(creds)) {
      const m = meta[id] || {}
      if (typeof v === 'string') {
        subscriptions.push({
          id,
      ...
      subscriptions.push({
        id,
        provider: v.provider || m.provider || inferProviderFromSub({ id }),
        label: v.label || m.label || id,
        kind: v.kind || '',
        fingerprint: m.fingerprint || v.fingerprint || null,
        credentialRef: v.credentialRef || m.credentialRef || '',
        card: m.card || null,
      })
    }
